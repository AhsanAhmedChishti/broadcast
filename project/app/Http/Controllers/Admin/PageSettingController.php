...
class PageSettingController extends Controller
{
    ...
    [
        ...
        'video_background'    => 'mimes:jpeg,jpg,png',
        'p_background'    => 'mimes:jpeg,jpg,png',
        'review_background'    => 'mimes:jpeg,jpg,png',
        'c_background'    => 'mimes:jpeg,jpg,png',
    ];
    ...
    public function about(Request $request)
    {
        if ('POST' === $request->method()) {
            $showTeamSection = 0;
            if ('on' === $request->toggle_team_section) {
                $showTeamSection = 1;
            }
            $request->merge(['toggle_team_section' => $showTeamSection]);
            AboutPageSetting::find(1)->update($request->except(['_token']));
            return redirect()->back()->with('status', ['success', 'Your changes have been saved.']);
        }

        $data = AboutPageSetting::find(1);
